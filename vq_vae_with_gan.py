...
class VQ_VAE(nn.Module):
	def __init__(self, hidden_size=256, num_embeddings=512, embedding_dim=256,bn=True, vq_coef=1, commit_coef=0.25, num_channels=3, use_wgan=True):
	...
	def loss(self, x, decoded_x, disc_factor):
		...
		# reconstruct loss
		r_loss = F.mse_loss(decoded_x, x) + F.l1_loss(decoded_x, x)
		# r_loss = F.l1_loss(decoded_x, x) + F.l1_loss(decoded_x, x) + 0.2 * self.wgan_gradient_loss(x, decoded_x)
	...
	# constrains discriminator grads to [-1,1] by sampling caclulation
	def wgan_gradient_loss(self, x, decoded_x):
		# Samples (B, 1, 1, 1)=>(16, 1, 1, 1) weights to line interpolation
		# Because we want to use same weight for both x and decoded_x, so samples (B,) instead of x.shape
		weights = torch.rand(x.shape[0], 1, 1, 1, requires_grad=True).to(device)

		# broadcast to (16, 3, 32, 32)
		i_x = weights * x + (1 - weights)* decoded_x

		# i_x.quire
		i_y = self.discriminator(i_x)

		i_grads = torch.autograd.grad(outputs=i_y, 
									inputs=i_x, 
									grad_outputs=torch.ones_like(i_y),
									create_graph=True, 
									retain_graph=True, 
									only_inputs=True)[0]
		
		return torch.square(i_grads.norm(2, dim=(2,3)) - 1).mean()
	...
	def test(self, epoch, load_model = False):
		...
		with torch.no_grad():
			for data, _ in self.test_loader:
				...
				# losses = self.model.loss(data, decoded_x, 1)
				...
				number_of_batches += 1
				# self.losses_trick.update_test(dict_refs, losses)

			# self.losses_trick.stat_test()
